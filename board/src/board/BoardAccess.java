...
public class BoardAccess implements BoardInterface{
	...
	boardList[0] = new Board(1, "제목01", "작성자01", "내용01", "2025/03/19 - 18:00:0",
														"2025/03/19 - 18:00:0");
	}
	...
	public Board create(Board board) {
		...
		int no = ++count;
		String createAT = "2025/03/19 - 18:00";
		String updateAT = "2025/03/19 - 18:00";
		board.setNo(no);;
		board.setCreateAt(createAT);
		board.setUpdatedAt(updateAT);
	 ...
	 * 2. 게시글 번호 no가 유효하면,
	 * 		반호에 해당하는 boardList[] 의 게시글 요소를 반환
	 */
	...
	/**
	 * 게시글 수정
	 * 1. 수정할 게시글 번호 no가 유효하지 않으면, false로 반환
	 * 2. 수정할 게시글 번호 no가 유효하면
	 * - 번호에 해당하는 boardList[]의 요소 변경
	 * - 수정일자(updateAt)를 현재 날짜/시간으로 변경
	 * - 수정 완료 후, true를 반환
	 */
	...
	/**
	 * 게시글 삭제
	 * 1. 삭제할 게시글 번호 no가 유효하지 않으면, false 반환
